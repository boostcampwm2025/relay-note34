...
AI한테 검토받았는데 완벽하다고 한다!ㅎㅎ

#### 느낀 점

처음에는 함수명과 변수명 몇 개만 바꾸려고 했었는데 하다 보니 매개변수에 있던 y도 arr로 확실히 한다면 이 함수를 보는 사람이 조금 더 함수 동작은 쉽게 파악할 수 있겠다는 생각이 들었다. 이번 릴레이 미션을 통해서 또 내 작은 사고의 틀을 깰 수 있었다. 그리고 항상 내 중심 코드를 짰었는데 내 코드는 누군가가 볼 수도 있는 코드라는 생각을 하며 명확한 설계를 해야겠다는 생각이 들었다. 
